<div class="card">
    ...
    <div class="card-body collapse">
        <table id="time_slots_table" class="table datatable-button-html5-columns">
            ...
            <tr>
                <th>{{ __('msg.sn') }}</th>
                <th>{{ __('msg.start_time') }}</th>
                <th>{{ __('msg.end_time') }}</th>
                <th>{{ __('msg.action') }}</th>
            </tr>
            </thead>
            <tbody>
                        ...
                        <div class="list-icons">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    {{--Edit--}}
                                    <a href="{{ route('ts.edit', $tms->id) }}" class="dropdown-item"><i class="icon-pencil"></i> {{ __('msg.edit') }}</a>

                                    {{--Delete--}}
                                    @if(Qs::userIsSuperAdmin())
                                        <a id="{{ $tms->id }}" onclick="confirmDelete(this.id)" href="#" class="dropdown-item"><i class="icon-trash"></i> {{ __('msg.delete_f2a6') }}</a>
