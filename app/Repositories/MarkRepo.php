...
class MarkRepo
{
    public function getGrade($total, $educational_stage_id)
    {
        if($total < 1) { return NULL; }

        $grades = Grade::where(['educational_stage_id' => $educational_stage_id])->get();
    ...
    {
        $grades = Grade::whereNull('educational_stage_id')->get();
        if($grades->count() > 0){
